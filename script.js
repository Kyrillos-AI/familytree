...
window.openBio = (id) => {
  ...
  /* =========================================
     [جديد] إضافة زر الانتقال للشجرة الأصلية
     ========================================= */
  // 1. تنظيف أي زر قديم (عشان ميتكررش)
  const existingVisitBtn = document.getElementById("visit-origin-tree-btn");
  if (existingVisitBtn) existingVisitBtn.remove();

  // 2. التحقق: هل هذا الشخص مربوط بشجرة خارجية؟
  if (m.isLinked && m.originalTreeId) {
    const actionsContainer = document.querySelector(".view-controls");

    const visitBtn = document.createElement("button");
    visitBtn.id = "visit-origin-tree-btn";
    visitBtn.className = "action-item";
    visitBtn.innerHTML = `
        <div class="icon-box" style="color: #6c5ce7; background: rgba(108, 92, 231, 0.1); border: 1px solid #6c5ce7;">
            🌍
        </div>
        <span>شجرة الأصل</span>
    `;

    // عند الضغط، ننفذ دالة الانتقال
    visitBtn.onclick = () =>
      window.visitExternalTree(m.originalTreeId, m.originalMemberId);

    // إضافة الزر للقائمة
    actionsContainer.appendChild(visitBtn);
  }
  // فتح المودال
...
window.searchForOccasionPartner = () => {
  ...
  if (matches.length > 0) {
   ...
   دالة حفظ المناسبة (مصححة وشاملة)
   ========================================= */
window.saveOccasion = async () => {
  ...
  // جلب معرف الشخص الحالي من المودال (أكثر أماناً)
  const focusId = document.getElementById("modal-id-display").innerText;

  if (!window.currentTreeId) return window.customAlert("خطأ في معرف الشجرة!");
  if (!type || !date) return window.customAlert("يرجى ملء كافة البيانات ⚠️");

  // 2. تحديد المصدر (هل هو شخص من العائلة أم جديد/خارجي؟)
  const sourceEl = document.querySelector(
    'input[name="partner-source"]:checked'
  );
  const source = sourceEl ? sourceEl.value : "existing";
  ...
  try {
    ...
    // =================================================
    // الحالة الأولى: شخص موجود داخل العائلة (Existing)
    // =================================================
    if (source === "existing") {
      partnerId = document.getElementById("occasion-partner-id").value;
      if (!partnerId)
        return window.customAlert("الرجاء اختيار الشريك من القائمة ⚠️");
    }
    // =================================================
    // الحالة الثانية: شخص جديد أو ربط خارجي (New / Global)
    // =================================================
    else {
      const nameVal = document.getElementById(
        "occasion-new-partner-name"
      ).value;
      if (!nameVal) return window.customAlert("الرجاء كتابة الاسم! ⚠️");

      // جلب بيانات الربط الخارجي من الحقول المخفية
      const extId = document.getElementById("ext-partner-id").value;
      const extTree = document.getElementById("ext-partner-tree-id").value;
      const extImg = document.getElementById("ext-partner-img").value;

      // تحديد الجنس (عكس جنس الشخص الحالي)
      const focusPerson = window.currentMembers.find((m) => m.id === focusId);
      const newGender = focusPerson.gender === "male" ? "female" : "male";
      const defaultImg =
        newGender === "female" ? "mainfemale.png" : "mainmale.png";

      // تجهيز بيانات العضو الجديد (سواء حقيقي أو بروكسي للربط)
      const newMemberData = {
        name: nameVal,
        gender: newGender,
        img: extImg || defaultImg,
        isPrivate: true, // غالباً الأنسباء لا يظهرون في الشجرة الرئيسية كأصول
        level: focusPerson.level,
        createdAt: new Date().toISOString(),

        // نربطه فوراً عند الإنشاء
        spouse: type === "marriage" ? focusId : null,
        fiance: type === "engagement" ? focusId : null,
        marriageDate: type === "marriage" ? date : null,
        engagementDate: type === "engagement" ? date : null,
      };

      // لو فيه بيانات ربط خارجي، نضيفها
      if (extId && extTree) {
        newMemberData.isLinked = true;
        newMemberData.originalTreeId = extTree;
        newMemberData.originalMemberId = extId;
        newMemberData.note = "تم الربط من شجرة خارجية 🌍";
      }

      // الحفظ في قاعدة البيانات
      const docRef = await addDoc(membersColl, newMemberData);
      partnerId = docRef.id;
    }

    // =================================================
    // 3. تحديث الشخص الحالي (صاحب البروفايل)
    // =================================================
    const updateData = {};
    if (type === "marriage") {
      updateData.spouse = partnerId;
      updateData.marriageDate = date;
      updateData.fiance = null; // فسخ الخطوبة إن وجدت
      updateData.engagementDate = null;
    } else {
      updateData.fiance = partnerId;
      updateData.engagementDate = date;
    }
    ...
    // =================================================
    // 4. تحديث الشريك (فقط لو كان موجوداً في شجرتنا)
    // =================================================
    // (لو كان جديداً، قمنا بربطه أثناء الإنشاء في الخطوة السابقة)
    if (source === "existing") {
      const partnerUpdate = {};
      if (type === "marriage") {
        partnerUpdate.spouse = focusId;
        partnerUpdate.marriageDate = date;
        partnerUpdate.fiance = null;
        partnerUpdate.engagementDate = null;
      } else {
        partnerUpdate.fiance = focusId;
        partnerUpdate.engagementDate = date;
      }
      await updateDoc(
        doc(db, "trees", window.currentTreeId, "members", partnerId),
        partnerUpdate
      );
    }
    ...
    const msg =
      source === "existing"
        ? "تم الارتباط بنجاح! ❤️"
        : "تم إضافة الشريك والربط بنجاح! 🌍";
    window.customAlert(msg);
    window.closeModalSmoothly("occasion-modal");

    // تفريغ الحقول المخفية استعداداً للمرة القادمة
    document.getElementById("ext-partner-id").value = "";
    document.getElementById("ext-partner-tree-id").value = "";
    document.getElementById("ext-partner-img").value = "";

    window.openBio(focusId); // تحديث الواجهة
  } catch (e) {
    console.error(e);
    window.customAlert("حدث خطأ: " + e.message);
  }
...
/* =========================================
   تحميل قائمة المحادثات الخاصة (مصحح)
   ========================================= */
function loadPrivateChatsList() {
  ...
  // ملاحظة: تأكد من إنشاء Index في الفايربيز لهذا الاستعلام إذا ظهر خطأ في الكونسول
  const q = query(
  ...
  onSnapshot(q, (snapshot) => {
    ...
    snapshot.forEach((doc) => {
      ...
      // [تصحيح] تم حذف السطر الذي كان يسبب العطل (memberTreeId)
      // الشات الخاص الآن عالمي ولا يحتاج لفلترة الشجرة
      ...
      if (data.usersInfo && data.usersInfo[otherUid]) {
        ...
        // محاولة البحث المحلي (للتوافق القديم)
        if (window.currentMembers) {
...
/* =========================================
   دالة الإرسال الموحدة (مصححة)
   ========================================= */
window.sendMessage = async (type) => {
  // التأكد من النوع، لو لم يتم تمريره نعتبره خاص (لتفادي الأخطاء)
  if (!type) type = currentChatRoomId ? "private" : "global";

  const inputId = type === "global" ? "global-input" : "private-input";
  const input = document.getElementById(inputId);

  if (!input) return; // أمان إضافي
  ...
  try {
    if (type === "global") {
      // إرسال للشات العام
      // صلاحية الرسالة 7 أيام
      msgData.expiresAt = new Date(Date.now() + 604800000).toISOString();

      if (!window.currentTreeId) return alert("خطأ: شجرة غير محددة");
      ...
      // 2. تحديث بيانات الغرفة (آخر رسالة)
    ...
    // [تصحيح] إغلاق الإيموجي هنا بدلاً من دالة التغليف الخارجية
    const emojiPopup = document.getElementById("emoji-popup");
    if (emojiPopup) emojiPopup.classList.remove("show");
  } catch (e) {
    console.error("Send Error:", e);
    window.customAlert("فشل الإرسال: " + e.message);
  }
};
...
/* =========================================
   Global Spouse Search Integration 🌍💍
   ========================================= */

let globalSpouseTimeout;

// 1. دالة البحث في كل العائلات
window.handleGlobalSpouseSearch = (term) => {
  clearTimeout(globalSpouseTimeout);
  const resultsDiv = document.getElementById("global-spouse-results");

  if (term.length < 3) {
    resultsDiv.innerHTML =
      '<div style="padding:10px; text-align:center; color:#aaa;">اكتب 3 حروف على الأقل...</div>';
    resultsDiv.style.display = "block";
    return;
  }

  resultsDiv.innerHTML =
    '<div style="padding:10px; text-align:center;">⏳ جاري البحث في السجلات...</div>';
  resultsDiv.style.display = "block";

  globalSpouseTimeout = setTimeout(async () => {
    try {
      const { collectionGroup, query, where, getDocs, limit } = await import(
        "https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js"
      );

      // بحث في كل الكولكشنز اللي اسمها "members" في قاعدة البيانات
      const q = query(
        collectionGroup(db, "members"),
        where("name", ">=", term),
        where("name", "<=", term + "\uf8ff"),
      ...
      if (snapshot.empty) {
        resultsDiv.innerHTML =
          '<div style="padding:10px; text-align:center;">لا توجد نتائج مطابقة 🤷‍♂️<br><small>يمكنك كتابة الاسم يدوياً بالأسفل</small></div>';
        // تفعيل الكتابة اليدوية لو ملقاش حد
        document.getElementById("occasion-new-partner-name").readOnly = false;
        document.getElementById("occasion-new-partner-name").focus();
        return;
      }

      snapshot.forEach((docSnap) => {
        const member = docSnap.data(); // المتغير هنا اسمه member

        // استخراج معرف الشجرة
        const pathSegments = docSnap.ref.path.split("/");
        const originTreeId = pathSegments[1]; // المتغير هنا اسمه originTreeId

        // استثناء أعضاء شجرتي الحالية
        if (originTreeId === window.currentTreeId) return;

        const div = document.createElement("div");
        div.className = "modal-search-item";

        // التصحيح:
        // 1. استبدال data.img بـ member.img
        // 2. استبدال data.name بـ member.name
        // 3. استبدال memberTreeId بـ originTreeId
        // 4. استبدال doc.id بـ docSnap.id

        div.innerHTML = `
        <div style="display:flex; align-items:center; gap:10px;">
            <img src="${
              member.img || "mainmale.png"
            }" style="width:40px; height:40px; border-radius:50%; object-fit:cover;">
            <div>
                <div style="font-weight:bold; color:white;">${member.name}</div>
                <div style="font-size:0.7rem; color:#aaa;">من شجرة: ${originTreeId}</div>
            </div>
        </div>
        <button class="btn-primary" style="padding:5px 10px; font-size:0.8rem; background:#10b981; border:none; border-radius:5px; color:white; cursor:pointer;"
            onclick="window.selectGlobalResult('${originTreeId}', '${
          docSnap.id
        }', '${member.name}', '${member.img || "mainmale.png"}')">
            ارتباط 🔗
        </button>
    `;
        ...
        div.onclick = () => {
          ...
          document.getElementById("occasion-new-partner-name").value =
            member.name;
          document.getElementById("ext-partner-id").value = docSnap.id;
          document.getElementById("ext-partner-tree-id").value = originTreeId;
          document.getElementById("ext-partner-img").value = member.img || "";

          // إخفاء النتائج وتجميل الواجهة
          resultsDiv.style.display = "none";
          document.getElementById("global-spouse-search").value = member.name;
          window.customAlert(`تم تحديد: ${member.name} ✅`);
        };
      ...
      if (e.message.includes("indexes")) {
        resultsDiv.innerHTML =
          '<div style="color:orange; padding:10px; font-size:0.8rem;">⚠️ يحتاج المطور لتفعيل الفهرس (Index) في الفايربيز ليعمل البحث العالمي.</div>';
      } else {
        resultsDiv.innerHTML =
          '<div style="color:red; padding:10px;">خطأ في البحث</div>';
      }
    }
  }, 800);
};
...
/* =========================================
   دالة حفظ المناسبة (النسخة النهائية المصححة)
   ========================================= */
window.saveOccasion = async () => {
  // 1. جلب البيانات الأساسية
  const type = document.getElementById("occasion-type").value;
  ...
  // 2. تحديد المصدر (هل هو شخص من العائلة أم بحث خارجي/جديد؟)
  const sourceEl = document.querySelector(
    'input[name="partner-source"]:checked'
  );
  const source = sourceEl ? sourceEl.value : "existing";
  ...
  try {
    ...
    // =================================================
    // الحالة الأولى: اختيار شخص موجود (Existing)
    // =================================================
    if (source === "existing") {
      partnerId = document.getElementById("occasion-partner-id").value;
      if (!partnerId)
    ...
    // =================================================
    // 4. تحديث الشريك (فقط لو كان موجوداً مسبقاً في شجرتنا)
    // =================================================
    if (source === "existing") {
      const partnerUpdate = {};
      if (type === "marriage") {
...
// عشان نغير نص "شخص جديد" لـ "بحث خارجي"
const originalUpdateUI = window.updateOccasionUI;
window.updateOccasionUI = () => {
  originalUpdateUI();

  // تعديل النصوص ديناميكياً
  const newRadioLabel = document.querySelector(
    'input[name="partner-source"][value="new"] + .tech-radio-content span:last-child'
  );
  if (newRadioLabel) newRadioLabel.innerText = "بحث خارجي / جديد";

  const input = document.getElementById("occasion-new-partner-name");
  if (input) input.readOnly = true; // نمنع الكتابة المباشرة إلا لو البحث فشل
};
/* =========================================
   نظام الزواج من شجرة أخرى (Cross-Tree Linking)
   ========================================= */

// 1. دالة اختيار شخص من البحث العام (معدلة للفحص)
window.selectGlobalResult = async (targetTreeId, targetMemberId, name, img) => {
  // إظهار لودينج بسيط
  const inputField = document.getElementById("partner-name");
  inputField.value = "جاري فحص الحالة الاجتماعية...";
  inputField.disabled = true;

  try {
    // فحص حالة الشخص في شجرته الأصلية
    const targetRef = doc(db, "trees", targetTreeId, "members", targetMemberId);
    const targetSnap = await getDoc(targetRef);

    if (!targetSnap.exists()) {
      alert("❌ هذا الشخص لم يعد موجوداً.");
      resetPartnerInput();
      return;
    }
    ...
    // 🛑 فحص الموانع الشرعية (هل متزوج أو مخطوب؟)
    if (data.spouse) {
      alert(`⛔ لا يمكن اختيار ${name} لأنه/ا متزوج/ة بالفعل في شجرته/ا!`);
      resetPartnerInput();
      return;
    }
    if (data.fiance) {
      alert(`⛔ لا يمكن اختيار ${name} لأنه/ا مخطوب/ة حالياً!`);
      resetPartnerInput();
      return;
    }

    // ✅ الشخص متاح - نجهز البيانات للربط
    inputField.value = name; // عرض الاسم
    inputField.disabled = false;

    // نخزن بيانات الشجرة الخارجية في "السمات" (Attributes) للحقل عشان نستخدمها وقت الحفظ
    inputField.setAttribute("data-external-tree", targetTreeId);
    inputField.setAttribute("data-external-member", targetMemberId);
    inputField.setAttribute("data-external-img", img || "mainmale.png");
    inputField.setAttribute("data-is-external", "true");

    // نغلق البحث
    window.closeGlobalSearch();
    window.customAlert(`✅ تم اختيار ${name}. الشخص متاح للارتباط.`);
  } catch (e) {
    console.error(e);
    alert("حدث خطأ أثناء فحص بيانات الشخص.");
    resetPartnerInput();
  }
};

// دالة مساعدة لتفريغ الحقل لو حصل خطأ
function resetPartnerInput() {
  const input = document.getElementById("partner-name");
  input.value = "";
  input.disabled = false;
  input.removeAttribute("data-external-tree");
  input.removeAttribute("data-external-member");
  input.removeAttribute("data-is-external");
}
/* =========================================
   دالة زيارة شجرة خارجية (Cross-Tree Navigation)
   ========================================= */
window.visitExternalTree = async (targetTreeId, targetMemberId) => {
  if (!targetTreeId) return;

  const confirmVisit = confirm(
    "هل تريد الانتقال لعرض شجرة العائلة الأصلية لهذا الشخص؟ 🌍✈️"
  );
  if (!confirmVisit) return;

  try {
    window.customAlert("جاري السفر للشجرة الأخرى... ✈️");
    window.closeBio(); // إغلاق المودال الحالي

    // 1. تغيير الشجرة الحالية
    window.currentTreeId = targetTreeId;

    // 2. تحديد "نقطة التركيز" لتكون على الشخص الذي اخترناه
    // (هذا المتغير تستخدمه دالة renderPerspectiveTree لفتح الشجرة عليه مباشرة)
    window.lastFocusId = targetMemberId;

    // 3. تحميل بيانات الشجرة الجديدة
    // (الدالة loadTreeData موجودة عندك بالفعل وستقوم بتحديث الشاشة)
    loadTreeData(targetTreeId);
  } catch (e) {
    console.error(e);
    window.customAlert("تعذر الانتقال للشجرة: " + e.message);
  }
};
